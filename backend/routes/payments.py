...
def execute_payment(current_user):
    ...
    purchase = Purchase.query.filter_by(paypal_order_id=payment_id, user_id=current_user.id).first()
...
def get_purchase_history(current_user):
    ...
    return jsonify(
        {"success": True, "purchases": [p.to_dict() for p in purchases], "total_spent": round(total_spent, 2)}
    ), 200
