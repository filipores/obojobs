...
def register():
    ...
    if not email or not password:
        return jsonify({"error": t("auth.emailAndPasswordRequired")}), 400

    try:
        user = AuthService.register_user(email, password, full_name)
        return jsonify({"message": t("auth.registrationSuccessful"), "user": user.to_dict()}), 201
...
def reset_password():
    ...
    if not validation["valid"]:
        return jsonify({
            "error": t("auth.passwordNotMeetRequirements"),
    ...
    return jsonify({"message": t("auth.passwordResetSuccessful")}), 200
...
def logout():
    ...
    return jsonify({"message": t("auth.logoutSuccessful")}), 200
