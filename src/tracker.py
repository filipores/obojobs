...
class BewerbungsTracker:
    def __init__(self, json_path: str):
        ...
        if os.path.exists(csv_path) and not os.path.exists(json_path):
            ...
            os.remove(csv_path)
            print(f"✓ CSV-Daten nach JSON migriert und CSV gelöscht")
    ...
    def log_bewerbung(self, firma: str, position: str, ansprechpartner: str,
                     email: Optional[str] = None, quelle: Optional[str] = None,
                     status: str = "erstellt", pdf_pfad: Optional[str] = None,
                     notizen: Optional[str] = None, betreff: Optional[str] = None,
                     email_text: Optional[str] = None, links: Optional[Dict] = None):
        bewerbung = {
    ...
    def get_latest_bewerbung(self) -> Optional[Dict]:
